...
## 0 **Before start**
***
If you are familiar with English, go to [5 Reference](#anchor1). And this text is provided by [my Github repository](https://github.com/be-favorite/Tutorial_prophet)

<br>

## 1 **Introduction**
***
SIR 모형은 구획 모형(compartmental models)의 일종으로, 모집단(population)을 질병 상태와 (필요하다면) 다른 특성을 나타내는 구획으로 나눈다. 그리고, 이렇게 나뉜 각 구획간의 전이(transition)를 모형화하는 데에 수학적 방정식을 사용한다. 그래서, 구획 모형은 수학적 모형이라 표현하기도 한다. 이러한 구획 모형의 한 종류라 할 수 있는 SIR 모형은 전염병 모델링의 주춧돌 역할을 한다고 할 수 있다. 해당 모형에서 모집단에 포함되는 개인(individuals)들은 다음의 세 구획중 하나에 포함된다:

* Susceptible ($S$): 민감군, i.e. 감염 가능 군
* Infected ($I$): 감염군
* Recovered ($R$): 회복군

각 구획은 해당 구획의 개인이 평균적으로 취할 행동을 모형화 한다는 점에서 모집단 기반(population-based) 모형이라고도 한다. 아울러, 전염병의 경우 모집단의 모든 이들이 감염될 가능성이 있어 모집단 전체를 민감군으로 볼 수 있으므로, 모집단 기반 모형이라고 표현할 수 있다고 생각한다. SIR 모형은 실 생활에서 전염병의 유행을 모형화 하기에는 지나치게 간단한 모형이긴 하지만, SIR 모형을 초석으로 하여 더 복잡한 모형들을 설계할 수 있다. SIR 모형을 그림으로 간단하게 표현해보면 다음과 같다:
...
SIR 모형에서 나뉘는 각 구획은 모수 $\lambda$, $\gamma$에 의해 전이된다. 해당 모수가 의미하는 바와 이를 추정하는 식은 잠시 뒤에 좀 더 정확하게 설명하겠다. 

### 1.1 The assumptions of an basic SIR model {#anchor2}
기본적인(basic) SIR 모형은 모집단에 대해 다음과 같이 3가지 사항을 가정하여 전염병 모델링을 수행한다.

* 동질적 모집단: 각 구획의 개인들은 동일한 위험(hazards)에 노출됨
* 충분히 잘 혼합된(well-mixed) 모집단: 감염 가능 군에 속하는 모든 이들의 감염 위험(risk)은 동일
* 영구적 면역력(immunity): 전염병으로 부터 회복된 개인들의 면역력은 영구적

기본적 SIR 모형에 포함되는 마지막 가정은 시뮬레이션 기간동안 모집단이 변하지 않고 면역력이 안정적인 경우에 좋은 예측력을 보일 것이다. 위 세 가지 가정으로부터 우리는 다음의 미분 방정식을 세울 수 있다:
...
여기서 $S$, $I$, $R$은 각 구획 민감군(susceptible), 감염군(infected), 회복군(recovered)에 속하는 사람의 수를 나타내며, $S+I+R = N$에 해당한다. 즉, $N$은 모집단의 크기라 할 수 있다. 그리고, $\gamma$는 회복률을 나타내며, 즉, 회복군의 변화량($\frac{dR}{dt}$)은 회복률 $\gamma$에 감염군의 수 $I$를 곱한 형태로 나타난다. $\beta$는 감염률을 나타낸다. $\beta$를 이용하면 민감군의 변화량($\frac{dS}{dt}$)은 감염률 $\beta$과 모집단의 크기 대비 감염군 수의 비율($\frac{I}{N}$)를 곱한 형태에 추가적으로 민감군의 수 $S$를 곱한 값으로 나타낼 수 있으며, 감염군의 변화량($\frac{dI}{dt}$)의 경우는 민감군의 변화량에 회복군의 변화량을 뺀 형태로 나타날 수 있다.

### 1.2 기초감염 재생산수(basic reproduction number, $R_0$)
기본 재생산 번호는 완전히 취약한 모집단에서 단일 감염 사례에 의해 발생하는 2차 감염의 평균 수입니다.
...
감염력이란, 단위시간 당 한 개인이 감염될 위험을 의미한다. 좀 더 풀어서 말하면, 민감군에 속하는 이들이 감염군으로 전이될 가능성을 의미한다. 감염력은 시간에 따라 변화한다. 왜냐하면, 유행의 초기에는 감염된 사람이 많이 없기때문에 감염력이 작겠지만 유행이 커질 수록 감염력도 커지기 때문이다. 이러한 감염력은 감염률 $\beta$과 모집단의 크기 대비 감염군 수의 비율($\frac{I}{N}$)를 곱한 형태로 주어진다. [1.1](#anchor2)에서 민감군의 변화량 계산에 사용되었던 값에 해당한다.
...
### 2.1 모집단 회전 모형화
많은 사례에서 면역을 얻은 사람들은 곧 신생아로 대체되며, 이들은 민감군에 속하게 된다. 따라서, 이러한 점을 모형에 반영할 필요가 있고, 모형화 과정을 간단하게 하기위해 질병으로부터 발생하는 사망은 무시한다.
...
## 4 **Summary**
***
이번 글에서는 기본적인 SIR 모형과 여러 시나리오를 모형화할 수 있게 해주는 SIR 모형의 확장에 대해 소개하였다. 앞서 소개한 모든 매커니즘들을 결합하면, 집단 면역을 이루기 위한 인구 대비 백신 접종 비율 등과 같은 백신 접종 계획의 설립에 상당한 도움이 될 수 있다. 사실 실제 상황에서 좀 더 디테일한 시나리오 연구를 위해서는 이보다 더 복잡한 모형이 필요로 되지만, 전염병의 유행을 모델링하는 모형의 근본적 토대는 오늘 소개한 것에서 크게 벗어나지 않는다. 이 글을 잘 이해했다면, 더 복잡한 모형을 이해하는 것에도 큰 문제는 없을 것이라 생각한다.`r emo::ji("blush")`
